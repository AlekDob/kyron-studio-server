// Route SSE dell'agente Statistiche (Ada). Stesso protocollo di
...
import { runStatsAgent } from "./agent.js";

export const statsAgentRoute = new Hono();

statsAgentRoute.use("*", tenantMiddleware);
statsAgentRoute.use("*", studioAuthMiddleware);

statsAgentRoute.post("/", async (c) => {
  ...
  return streamSSE(c, async (stream) => {
    try {
      for await (const chunk of runStatsAgent({
        tenant,
        cookie,
