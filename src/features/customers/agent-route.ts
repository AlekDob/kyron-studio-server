// Route SSE dell'agente Clienti (Bea). Stesso protocollo di
...
import { runCustomersAgent } from "./agent.js";

export const customersAgentRoute = new Hono();

customersAgentRoute.use("*", tenantMiddleware);
customersAgentRoute.use("*", studioAuthMiddleware);

customersAgentRoute.post("/", async (c) => {
  ...
  return streamSSE(c, async (stream) => {
    try {
      for await (const chunk of runCustomersAgent({
        tenant,
        cookie,
