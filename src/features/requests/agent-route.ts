// Route SSE dell'agente Richieste (Ivo). Stesso protocollo di
...
import { runRequestsAgent } from "./agent.js";

export const requestsAgentRoute = new Hono();

requestsAgentRoute.use("*", tenantMiddleware);
requestsAgentRoute.use("*", studioAuthMiddleware);

requestsAgentRoute.post("/", async (c) => {
  ...
  return streamSSE(c, async (stream) => {
    try {
      for await (const chunk of runRequestsAgent({
        tenant,
        cookie,
